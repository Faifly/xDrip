...
final class GlucoseReading: Object {
    ...
    static func lastReadings(_ amount: Int, forMaster: Bool) -> [GlucoseReading] {
        return last(forMaster: forMaster, amount: amount, filter: { $0.calculatedValue !~ 0 && $0.rawValue !~ 0 })
    }
    
    static func latestByCount(_ amount: Int, forMaster: Bool) -> [GlucoseReading] {
        return last(forMaster: forMaster, amount: amount, filter: { $0.rawValue !~ 0 })
    }
    
    static func last(forMaster: Bool, amount: Int, filter: (GlucoseReading) -> Bool) -> [GlucoseReading] {
        guard amount > 0 else { return [] }
        
        let allReadings = forMaster ? allMasterForCurrentSensor.filter(filter) : allFollower.filter(filter)
    ...
    static func estimatedRawGlucoseLevel(date: Date) -> Double {
        guard let last = lastReadings(1, forMaster: true).first else { return 160.0 }
    ...
    func findSlope() {
        let last2Readings = GlucoseReading.lastReadings(2, forMaster: true)
