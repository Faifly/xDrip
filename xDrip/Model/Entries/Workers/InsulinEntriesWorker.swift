...
final class InsulinEntriesWorker: AbstractEntriesWorker {
    ...
    private static func calculateValue(prevEntry: InsulinEntry?, toDate: Date?, lastValue: inout Double) {
        ...
        if basalRates.count == 1 {
            ...
            let diff = unitsPerHour * date.timeIntervalSince(prevDate).hours
            ...
            var rates = calculateRates(startTime: prevEntryStartTime, endTime: currentEntryStartTime)
    ...
    private static func calculateRates(startTime: TimeInterval, endTime: TimeInterval) -> [BasalRate] {
        let basalRates = User.current.settings.sortedBasalRates
