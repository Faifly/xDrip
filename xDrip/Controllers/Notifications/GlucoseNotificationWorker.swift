...
final class GlucoseNotificationWorker: NSObject {
    ...
    private func resetMissedReadingsTimer() {
        let readings = GlucoseReading.lastValidReadings(4, for: User.current.settings.deviceMode)
    ...
    private func isGlucoseChangingFast(isRise: Bool) -> Bool {
        let last3 = GlucoseReading.latestValidByCount(3, for: User.current.settings.deviceMode)
