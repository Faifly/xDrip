...
final class GlucoseNotificationWorker: NSObject {
    ...
    private func resetMissedReadingsTimer() {
        let readings = GlucoseChartEntry.lastReadings(4, for: User.current.settings.deviceMode)
    ...
    private func isGlucoseChangingFast(isRise: Bool) -> Bool {
        let last3 = GlucoseChartEntry.latestByCount(3, for: User.current.settings.deviceMode)
